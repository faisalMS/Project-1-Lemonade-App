...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        when (lemonadeState) {
            ...
            SELECT -> { lemonadeState = SQUEEZE ;lemonSize = lemonTree.pick() ;squeezeCount = 0 }
            ...
            SQUEEZE -> { squeezeCount += 1 ;lemonSize -= 1 ;if (lemonSize == 0) { lemonadeState = DRINK ;lemonSize = -1} }
    ...
    private fun setViewElements() {
        ...
        when(lemonadeState){
            SELECT -> { textAction.text = "اقطف من شجرة الليمون \uD83C\uDF4B" ; lemonImage?.setImageResource(R.drawable.lemon_tree) }
            SQUEEZE -> { textAction.text = "اعصر الليمون" ;lemonImage?.setImageResource(R.drawable.lemon_squeeze) }
            DRINK -> { textAction.text = "اشرب الليمون \uD83E\uDD64" ;lemonImage?.setImageResource(R.drawable.lemon_drink) }
            RESTART -> { textAction.text = "خلص العصير\uD83D\uDE05 عيد التجربة" ;lemonImage?.setImageResource(R.drawable.lemon_restart) }
        }
    }
